...
DiscussionStatement.objects.filter(opinion_space=first_os).update(is_current=False)

s = "In what specific way would you improve this course this semester or in the future?"
d = DiscussionStatement(opinion_space=first_os, is_current=True, statement=s, short_version=s)
d.save()
...
comments = [
    "I would like lots of clarity on the rubric for grading our assignments",
    "I hope we receive plenty of feedback on what to improve from graders on our homework and exams.",
    "Can we post any required or suggested readings in advance, so we can look over them before lecture?",
    "I really enjoy working in groups and am looking forward to that in this class",
    "I find Piazza to be a really helpful and fast way for asking questions to instructors and classmates.",
]
...
for i in range(0, 5):
    ...
    a = AdminCommentTag(comment=comment, tag=tags[i])
    a.save()
