...
from opinion.includes.pca_scikit import PrincipalComponentAnalysis as pca_scikit
...
for rating in os.ratings.filter(is_current = True):
    try:
        ...
        col = rating.opinion_space_statement.statement_number-1
...
print "shape[0]=" + str(data.shape[0])
print str(data)

pca = pca_scikit(data, 2)
sorted_eigenvectors = pca.runAlgorithm()
    #standardized = True
