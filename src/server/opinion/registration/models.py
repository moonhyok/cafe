    ...
    def create_inactive_user(self, username, password, email,
        ...
        if send_email:
            ...
            message = render_to_string('registration/activation_email.txt',
                                       { 'url_root': settings.URL_ROOT, # Added for Opinion Space
                                         'activation_key': registration_profile.activation_key,
                                         'expiration_days': settings.ACCOUNT_ACTIVATION_DAYS,
                                         'site': current_site })
            
            send_mail(subject, message, Settings.objects.string('DEFAULT_FROM_EMAIL'), [new_user.email])
        return new_user
    
    def create_active_user(self, username, password, email, first_name,
                           send_email=True, profile_callback=None):
        """
        Added for Opinion Space:
        Creates an already-activated new user (no activation necessary)
        """
        new_user = User.objects.create_user(username, email, password)
